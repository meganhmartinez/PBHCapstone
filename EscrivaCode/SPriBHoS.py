...
def epsilon_horizon_crossing(rmNt):
...
#we build the curvature profile \bar{K}. and its derivative, needed to build the initial conditions
	...
	while t<t_final: #we set a maximum time (an "exit") to avoid problems
		...
		if t>tH:
			...
			if (np.any(np.isnan(Ak)) == True) or (np.any(np.isnan(Uk)) == True) or (np.any(np.isnan(Mk)) == True) or (np.any(np.isnan(ek)) == True) or (np.any(np.isnan(Gk)) == True) or (np.any(np.isnan(Rk)) == True):
				print ("The simulation has broken before the time of horizon crossing, probably because the initial conditions are wrong, the curvature profile is problematic of the stability condition between Ncheb and dt0 is not satisfied, check it")
				sys.exit(0)
...
epsrr,tH,rmww = epsilon_horizon_crossing(rm_N) #we get the epsilon parameter
